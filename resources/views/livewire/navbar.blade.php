<div class="max-w-full w-full" id="father-menu">
    <header>
        <div id="anside-menu"
            class="bg-white w-9/12 sm:w-4/12 min-h-lvh h-full fixed left-0 z-0 lg:hidden shadow-xl hidden justify-start flex-col items-end">
            <svg viewBox="0 0 24 24" width="32" height="62" fill="#0000" xmlns="http://www.w3.org/2000/svg"
                onclick="hideMenu()">
                ...
                <g id="SVGRepo_iconCarrier">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                        ...
                        fill="#0F1729" data-darkreader-inline-fill="" style="--darkreader-inline-fill: #0c1221;"></path>
                </g>
            </svg>
            <ul class="list-none lg:flex justify-center items-start flex-col self-start">
                @foreach ($navbarCategory as $category)
                    <li class="uppercase hover:underline text-black pl-8 font-roboto font-bold shadow-2xl my-1.5">
                        {{ $category->name }}</li>
                @endforeach
            </ul>
        </div>
        <nav class="bg-custom-black flex flex-col justify-center items-center">
            <div
                class="sm:max-w-screen-xl w-full flex sm:justify-around justify-between items-center m-auto  min-h-9 h-full ">
                ...
                <form method="POST" action={}
                    class="lg:max-w-xl sm:max-w-sm w-full h-full hidden justify-center justify-end items-center sm:flex ">
            ...
            <div class="w-full flex justify-center justify-end items-center sm:hidden">
                <form method="POST" action={} class="w-full flex items-center sm:m-2.5 p-1">
                    <input type="text" placeholder="O que você esta procurando?"
                        class="w-full text-center max-h-6 h-full" />
                    <button type="submit">
                        <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"
        ...
        <nav class="min-h-8 h-full w-full shadow-md flex justify-center items-center bg-white">
            <div class="sm:max-w-screen-xl w-full flex lg:justify-around justify-end items-center">
                <ul class="list-none lg:flex hidden justify-center items-start   ">
                    @foreach ($navbarCategory as $category)
                        <li class="uppercase hover:underline text-black pl-8 font-roboto font-bold shadow-2xl">
                            {{ $category->name }}</li>
                    @endforeach
                </ul>
...
<script>
    ...
    const divFatherMenu = document.getElementById('father-menu');
    ...
    window.onscroll = function() {
        const menu = document.getElementById("father-menu");
        
        if (window.scrollY > 60) {
            menu.classList.add("fixed", "z-50");
        } else {
            menu.classList.remove("fixed", "z-50");
        }
    };
</script>
